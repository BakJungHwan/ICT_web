<?php
      ...
      $count_sql = "SELECT count(*) as line_count FROM id_board";
      ...
      $sql = 'UPDATE id_board SET count=count+1 WHERE id = '.$_GET['article_id'];
      mysqli_query($conn,$sql); # 조회수 +1

      $sql = 'SELECT * FROM id_board where id='.$_GET['article_id'];
  ...
  <table border='0' width= '50%' height='50%' bgcolor='ccddd' align='center'>
        ...
        <tr>
          <td width='10%' height='10' align='right' valign='top' bgcolor='ccddd' colspan='2'>
            <a href="./chk_data.php?num=<?=$_GET['num']?>&select=1&article_id=<?=$_GET['article_id']?>">수정</a>
            <a href="./chk_data.php?num=<?=$_GET['num']?>&select=2&article_id=<?=$_GET['article_id']?>" onclick="return confirm('정말 삭제하시겠습니까?')">삭제</a>
          </td>
        </tr>
